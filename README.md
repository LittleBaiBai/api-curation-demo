...
1. Update TAP GUI values so guest login is enabled

```bash
kubectl edit secret tap-tap-install-values -n tap-install
```

1. Create SSO secret for gateway

```bash
kubectl apply -f "config/sso/sso-config.yaml"

# wait for k8s resource to appear, otherwise `kubectl wait` will fail on non-existent resource
timeout --kill-after=30s 30s bash -c "while ! kubectl get -n appsso deployment/authserver-auth-server; do sleep 5; done"
kubectl wait --for=condition=available -n appsso deployment/authserver-auth-server --timeout=60s
kubectl wait --for=condition=available -n appsso deployment/authserver-redis --timeout=60s
kubectl get deploy -n appsso

# the client secret is generated by appsso after apply the file above
export clientSecret="$(kubectl get secret client-registration -n appsso -o json  | jq -r '.data."client-secret"' | base64 -d)"

kubectl create secret -n default generic sso-credentials \
  --from-literal=client-id=appsso_client-registration \
  --from-literal=client-secret=$clientSecret \
  --from-literal=scope=openid,profile \
  --from-literal=issuer-uri=http://appsso.appsso
```
...
1. Update API Auto Registration with SCG is enabled
